...
const sora_1 = require("../lib/integrations/sora");
const veo_1 = require("../lib/integrations/veo");
const storage_1 = require("../lib/storage");
const videoJobs_1 = require("../lib/repositories/videoJobs");
const logger_1 = require("../lib/logger");
/**
 * Video Generation Worker
 * Processes video generation jobs from BullMQ queue
 */
const logger = (0, logger_1.createLogger)({ service: 'video-worker' });
// Configuration
...
async function processVideoJob(job) {
    ...
    try {
        ...
        await (0, videoJobs_1.updateVideoJob)(job.data.jobId, {
            status: 'processing',
        });
        ...
        const videoResult = await pollForCompletion(job, providerJobId, job.data.provider, job.data.apiKey, jobLogger);
        ...
        const videoBuffer = await (0, storage_1.downloadVideo)(videoResult.videoUrl);
        ...
        const uploadResult = await (0, storage_1.uploadVideo)(videoBuffer, {
 ...
 */
async function initiateVideoGeneration(data, logger) {
    if (data.provider === 'sora') {
        ...
        const result = await (0, sora_1.generateSoraVideo)(data.apiKey, request);
        return result.jobId;
    }
        ...
        const request = {
            ...
            duration: Math.min(data.duration, 8), // Veo max is 8 seconds
            fps: 24,
        };
        const result = await (0, veo_1.generateVeoVideo)({ projectId, apiKey: data.apiKey }, request);
        return result.jobId;
    }
    else {
 ...
 */
async function pollForCompletion(job, providerJobId, provider, apiKey, logger) {
    let attempts = 0;
    while (attempts < MAX_POLLING_ATTEMPTS) {
        ...
        try {
            ...
            const progress = Math.min(Math.floor((attempts / MAX_POLLING_ATTEMPTS) * 90), 90);
    ...
    throw new Error(`Video generation timed out after ${MAX_POLLING_ATTEMPTS} attempts (${(MAX_POLLING_ATTEMPTS * POLLING_INTERVAL_MS) / 1000}s)`);
}
/**
 * Check video status with the appropriate provider
 */
async function checkProviderStatus(provider, providerJobId, apiKey) {
    if (provider === 'sora') {
        return await (0, sora_1.getSoraVideoStatus)(apiKey, providerJobId);
    }
    else if (provider === 'veo') {
        const projectId = process.env.GOOGLE_CLOUD_PROJECT_ID || '';
        return await (0, veo_1.getVeoVideoStatus)({ projectId, apiKey }, providerJobId);
    }
    else {
 ...
 */
const worker = (0, queue_1.createVideoGenerationWorker)(processVideoJob);
// Event handlers
...
//# sourceMappingURL=video-generation-worker.js.map