import { SoraPreset } from './types';

export const SORA_PRESETS: SoraPreset[] = [
  {
    key: 'cinematic',
    label: 'Cinematic',
...
export function getPresetByKey(key: string): SoraPreset | undefined {
  return SORA_PRESETS.find(preset => preset.key === key);
}

export function getDefaultPreset(): SoraPreset {
  return SORA_PRESETS[0]; // Cinematic
}
